...
public interface IPatientOutputService
{
    ...
    Task SaveEventsLogAsync(string patientDir, string filePrefix, string examType, int scanlines, double peakKv, double elapsed, string? unitId = null, string? deviceHost = null, int phaseErr = 0, CancellationToken ct = default);
    Task AppendSessionAsync(string patientDir, string examType, double kvPeak, int scanlines, string? imageFile, string? eventsLog, string? dcmFile, string? unitId = null, string? deviceHost = null, int phaseErr = 0, CancellationToken ct = default);
}
